...
#include <stdio.h>
#include <random>
//This program simulates restricted diffusion with exchange in a substrate of spheres defined analytically
//Cylinders may have any packing and any size distribution
...
__device__ bool is_particle_in_any_cell(float tmp_x, float tmp_y, float tmp_z, world* w, float* centre_x, float* centre_y,float* centre_z, float* radii, long long* table, long long* cell_idx)
{
	long long x_pos = floor(tmp_x / w->vox_size);
	long long y_pos = floor(tmp_y / w->vox_size);
	long long z_pos = floor(tmp_z / w->vox_size);
	long long xyz; //voxel identifier
	long long which_cell, which_voxel;
	bool inside = false; //zero means outside all cells
	float distance, r, cx, cy, cz;
	
	pair(x_pos, y_pos, z_pos, xyz); //get the identifier
	
	which_voxel = binary_search_iter(table, 0, w->num_voxels - 1, xyz); //iterative binary search	
    ...
    cz = centre_z[which_cell];
	
	if (which_cell >= 0) //-1 means voxel is not in any cell
	{
		distance = (tmp_x-cx)*(tmp_x-cx) + (tmp_y-cy)*(tmp_y-cy) + (tmp_z-cz)*(tmp_z-cz);
...
__device__ void check_state(world* w, options* opt, float& tmp_x, float& tmp_y, float& tmp_z, float& tmp_dx,
	float& tmp_dy, float& tmp_dz, int& tmp_loc, float* centre_x, float* centre_y,  float* centre_z, float *radii, long long* table, long long * cell_idx,  curandState* states, int index)
{
	bool reject = false; //reject move or not
	bool inside;

	inside = is_particle_in_any_cell(tmp_x, tmp_y, tmp_z, w, centre_x, centre_y, centre_z, radii, table, cell_idx); //determine whether particle is in any cell
...
void engine(float* x, float* y, float* z, int* loc, float* centre_x, float* centre_y,  float* centre_z, float* radii, long long* table, long long* cell_idx,
	...
	if (c_p < opt->Npart) //ensure we keep within bounds
	{
		...
		for (int c_t = 0; c_t < 50; c_t++)
		{
			...
			check_state(w, opt, tmp_x, tmp_y, tmp_z, tmp_dx,
				tmp_dy, tmp_dz, tmp_loc, centre_x, centre_y,centre_z, radii, table, cell_idx, states, index);
			restrict_to_world(e_x, e_y, e_z, opt, w, tmp_x, tmp_y, tmp_z);
		}
...
bool load_substrate(float* h_centre_x, float* h_centre_y, float* h_centre_z, float* h_radii, long long* h_table, long long *h_cell_idx, world* w, char* g_fn)
{
	FILE* sf;
	sf = fopen(g_fn, "rb");
	if (sf)
	...
	fread(h_centre_z, sizeof(float), w->num_cells, sf);
...
__global__ void generate_initial_distribution(float* x, float* y, float* z, int* loc, float* centre_x,
	float* centre_y, float* centre_z, float* radii, long long* table, long long* cell_idx, curandState* states, options* opt, world* w)
{
...
int main(void)
{
	...
	char r_fn[500], s_fn[500], g_fn[500]; //filenames for trajectories, states and substrate
	//options structure for host
	options* opt, * dev_opt;
	//declare substrate structure for device and host
	world* h_w, * dev_w;
	//declare substrate arrays on device and host
	float* h_centre_x, *h_centre_y,*h_centre_z, *h_radii;
	float* dev_centre_x, *dev_centre_y,*dev_centre_z, *dev_radii;
	...
	h_centre_z = (float*)malloc(h_w->num_cells * sizeof(float));
	...
	//load the substrate
	PROCEED = load_substrate(h_centre_x, h_centre_y, h_centre_z, h_radii, h_table, h_cell_idx, h_w, g_fn);
	}
	...
	cudaMalloc(&dev_centre_z, h_w->num_cells * sizeof(float));
	...
	if (error != cudaSuccess)
	{
		...
		cudaMemcpy(dev_centre_z, h_centre_z, h_w->num_cells * sizeof(float), cudaMemcpyHostToDevice);
	...
	generate_initial_distribution << < numBlocks, blockSize >> > (dev_x, dev_y, dev_z, dev_loc, dev_centre_x, dev_centre_y,dev_centre_z, dev_radii, dev_table, dev_cell_idx, dev_states, dev_opt, dev_w);
	...
	engine << < numBlocks, blockSize >> > (dev_x, dev_y, dev_z, dev_loc, dev_centre_x, dev_centre_y,dev_centre_z, dev_radii, dev_table, dev_cell_idx, dev_states, dev_opt, dev_w);
