...
exe_fn = 'simplex_ss.exe'; %executable name
...
opt.SUBSTRATE_FILE_NAME = '..\substrates\test_spheres.bin';
opt.GRADIENT_FILE_NAME = '..\gradients\test_gwf.bin';
opt.SIGNAL_FILE_NAME = '..\output\test_signal_ss.bin';
opt.SAVE_LAST_POSITIONS = 1; %1 = yes, 0 = no
opt.TRAJECTORY_FILE_NAME = '..\output\test_traj_ss.bin';
opt.SAVE_STATE_HISTORY = 1; %1 = yes, 0 = no
opt.STATE_FILE_NAME = '..\output\test_stat_ss.bin';
opt.SAVE_PHASE = 1; %1 = yes, 0 = no
opt.PHASE_FILE_NAME = '..\output\test_phase_ss.bin';
...
dt_new = 1e-5; %NOTE: using a coarse sampling resolution such as 1e-3 causes a visible bias in the predicted signals
...
for c_gwf = 1:Ngwf
    for c_bv = 1:Nb
        ...
        Dw = resex_mc_compute_dw_sphere(w, d, D0, num_k);
        dw = w(2)-w(1);
