...
bool load_gwf(char* gwf_fn, options* opt, float* h_gwf_x, float* h_gwf_y, float* h_gwf_z)
{
	...
	std::cout << "Number of time points in waveform = : " << opt->n_gwf_points << std::endl;
	return true;
	}
	else
	{
...
int main(void)
{
	...
	//load the substrate
	PROCEED = load_substrate(h_centre_x, h_centre_y, h_centre_z, h_radii, h_table, h_cell_idx, h_w, sub_fn);
	...
	engine << < numBlocks, blockSize >> > (dev_x, dev_y, dev_z, dev_loc, dev_centre_x, dev_centre_y, dev_centre_z, dev_radii, dev_table, dev_cell_idx, dev_states, dev_opt, dev_w, dev_phase,
